      ...
      <div class="row py-lg-5">
        <div class="col-lg-6 col-md-8 mx-auto">
          ...
          <div v-if="!citizenData">
            ...
            <input
              type="text"
              class="form-control border-primary mb-2 w-50 mx-auto"
              :placeholder="$t('GHENT_PORTAL.CITIZEN.personalIdentifier')"
              aria-label="PersonalID"
              v-model="personalID"
            />
            <button
              @click="initCitizenSignup"
          ...
          <div v-if="citizenData">
            ...
            <b-check
              v-if="sessionId == null"
              v-model="preAuthorized"
              class="mb-2"
              >Pre-authorized</b-check
            >
            <b-check
              v-if="sessionId == null"
              v-model="userPinRequired"
              class="mb-2"
              >Requires user PIN</b-check
            >
            <input
              :disabled="!userPinRequired"
              v-if="userPinRequired"
              type="password"
              class="form-control border-primary mb-2 w-50 mx-auto"
              placeholder="PIN"
              aria-label="PIN"
        ...
        <b-modal id="qr-modal" :static="true" centered>
          ...
          <div class="text-center" :v-show="qr - code - visible">
            ...
            <div class="text-center small">
              <a :href="walletUrl" :disabled="!canSubmit"
                ><i class="bi bi-app-indicator px-2"></i
                >{{ $t("WALLET_APP") }}</a
              ><br />
              <a @click="goToWallet('walt.id')" href="#" :disabled="!canSubmit"
                ><img v-if="btnLoading" src="loader.gif" width="20px" /><span
                  v-else
                  ><i class="bi bi-box-arrow-up-right px-2"></i
                  >{{ $t("GHENT_PORTAL.WEB_WALLET") }}</span
                ></a
              >
            </div>
          </div>
...
export default {
  ...
  computed: {
    canSubmit() {
      return (
        ...
        !this.btnLoading &&
        (!this.userPinRequired || this.userPin != null)
      );
    },
  },
  ...
  methods: {
    async goToWallet(walletId) {
      ...
      const params = {
        walletId: walletId,
        isPreAuthorized: this.preAuthorized,
        userPin: this.userPin,
      };
      if (walletId != "x-device") {
        window.location = `/ghent/signup/issue/${this.personalID}?${Object.keys(
          params
        )
        ...
        this.walletUrl = await this.$axios.$get(
          `/ghent/signup/issue/${this.personalID}`,
          { params: params }
        );
        new QRious({
    ...
    async initCitizenSignup() {
      this.citizenData = await this.$axios
        .$get(`/ghent/signup/${this.personalID}`)
        .catch((reason) => {
